...
#
    ...
    def hierarchicalCluster(self):
        linesAbove = [[gene, 1, False] for gene in self.distTable[0][:-1]]
        ...
        while len(self.distTable) > 2:
            ...
            print (merge)
            mergeIdxs = []
            for idx, (el, lines, merged) in enumerate(linesAbove):                
                if el not in merge and not merged:
                    linesAbove[idx][1] += 1
                else:
                    linesAbove[idx][2] = True
                    mergeIdxs.append(idx)

            linesAbove[mergeIdxs[0]][0] += ' '.join([gene for gene, lines, merged in linesAbove])
            del linesAbove[mergeIdxs[1]:mergeIdxs[-1]]
                
            #for i in range(len(self.distTable)):
                #print(self.distTable[i])
            #print("==========================================================================")
        print (linesAbove)
        linesAbove = sorted(linesAbove, key=lambda x: x[1])
        leafs = [gene for gene, lines, merged in linesAbove]
        print ('\t'.join(leafs))
        treeStrs = []
        while sum(lines for gene, lines, merged in linesAbove) > 0:
            lineStr = ""
            for idx, (gene, lines, merged) in enumerate(linesAbove):
                if lines > 1:
                    lineStr += '|\t'
                elif lines == 1:
                    lineStr += '|_______'
